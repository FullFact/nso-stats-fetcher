...
def fetch_ng_inflation_cpi():
    ...
    try:
        ...
        print("Failed to get/read Excel file")
        return

    # inflation stats don't start until 1997 so skip the rows before
    df.drop(df.loc[0 : df.index[df[0] == 1997][0] - 1].index, inplace=True)

    output_df = pandas.DataFrame(
        {"year": df.iloc[:, 0], "month": df.iloc[:, 1], "observation": df.iloc[:, 5]}
    )
    ...
    output_filepath = (
        filepaths.DATA_DIR / stats_metadata["NG"]["inflation"]["CPI"]["filename"]
    )
